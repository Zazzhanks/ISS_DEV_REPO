...
loader.load(
		...
		window.addEventListener("dblclick", function () {
			...
			if (is_iss_selected === false) {
				gsap.to(camera.position, {
					...
					x: center.x,
					y: center.y,
					z: center.z + 0.1, // maybe adding even more offset depending on your model
				});
	...
	function (xhr) {
		...
		if (Number((xhr.loaded / xhr.total) * 100) >= 99) {
			element.innerHTML =
				"ISS loaded. Double click anywhere to zoom ISS.";
		} else {
			element.innerHTML =
...
function render_line() {
	...
	for (var i = 0; i <= (90 || 1440 / 16); i += 1) {
		const date = new Date(new Date().getTime() + i * 80000);
	...
	scene.add(line);
	setInterval(scene.remove(line), 1000);
}
